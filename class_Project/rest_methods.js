...
app.post("/register", function(req, res) {
  ...
  if(!req.body)
    return res.status(ERROR_STATUS).send({'response': 'Error', 'message': 'Parameter is undefined'});
  ...
  connection.query(addNewUserQuery, function(err, results, fields) {
    if (err)
      ...
      return res.status(ERROR_STATUS).send({'response': 'Error', 'message': "DB `Error: " + err});
    else {
  ...
  connection.query(passwordAndSaltQuery, function(err, results, fields) {
    if(err)
      ...
      if(!results.length)//if the user does not exist in database
        return res.status(ERROR_STATUS).send({'response': 'Error', 'message': 'Incorrect username or password'});
      else {        
        //use sha512 and the salt
...
app.post("/sendMessage",function(req,res) {
  ...
  if(!req.body || !req.headers["token"])
    return res.status(ERROR_STATUS).send({'response': 'Error', 'message': "Wrong inputs"});

  var decoded ="";
  try {//verify the token
    decoded = jwt.verify(req.headers["token"], secret);
  } catch (Error) {//does not pass the verification, exit method
    return res.status(ERROR_STATUS).send({'response': 'Error', 'message': Error});
  }
    ...
    if (err)
      ...
      connection.query(deleteMyMessagesQuery, function(err, results, fields) {
        if(err)
          ...
          return res.status(SUCCESS_STATUS).json({'response': 'Success', 'message':'Sucessful' , 'messagecount': i, "messages": jsonArray});
      });
    }
  });  
});
